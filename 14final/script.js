...
function updateWarCost() {
  ...
  taxes = taxes - warCost;
  warCost = ruled * 50;
  if (ruled >= numberKingdoms) {
    //checks to see if the number of kingdoms ruled is greater than the
    //number of all available kingdoms
    //declares win if they are
    //disables the button on the form
    document.getElementById("ruleworld").innerHTML =
       "Congratulations!! You rule the world!!";
    document.getElementById("war").disabled = true;
    document.getElementById("war").style.backgroundColor = "#948caa";
  }
...
function displayCrowns() {
  ...
  if (jewelsOwned >= maxJewels) {
    ...
    document.getElementById("buy-jewels").style.backgroundColor = "#948caa";
  }
}
...
function initializeForm() {
  ...
  document.getElementById("buy-jewels").style.backgroundColor = "#dda0dd";
  document.getElementById("ruleworld").innerHTML = "";
  document.getElementById("war").disabled = false;
  document.getElementById("war").style.backgroundColor = "#dda0dd";
